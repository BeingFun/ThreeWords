...
def get_font_dict():
    font_dict_sort = {}
    font_dict_path = Config.ROOT_PATH + r"\config\font_dict.json"
    # 如果文件存在且不为空,则返回
    if os.path.exists(font_dict_path):
        if os.stat(font_dict_path).st_size != 0:
            with open(font_dict_path, "r", encoding="utf-8") as load_file:
                font_dict_sort = json.load(load_file)
            return font_dict_sort
    else:
        # 创建文件
        with open(font_dict_path, "w", encoding="utf-8") as load_file:
            pass

    # 如果问价不存在或为空则,重新获取系统字体
    font_dict = {}
    ttflist = FontManager().ttflist
    for f in ttflist:
        if Path(f.fname).suffix.lower() == '.ttf':
            ...
            if out_ttf:
                font_dict[out_ttf] = f.fname
            else:
                font_dict[f.name] = f.fname
            ...
            for ttf in ttc:
                ...
                if out_ttc:
                    font_dict[out_ttc] = f.fname
                else:
                    font_dict[f.name] = f.fname
    # 按字体名排序
    keys = sorted(font_dict.keys())
    for key in keys:
        font_dict_sort[key] = font_dict[key]
    # 写入文件
    font_dict_path = Config.ROOT_PATH + r"\config\font_dict.json"
    with open(font_dict_path, "w", encoding="utf-8") as dump_file:
        json.dump(font_dict_sort, dump_file, ensure_ascii=False)
    return font_dict_sort
